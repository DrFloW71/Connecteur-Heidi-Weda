...
(function () {
    ...
    const VERSION_AUTO_ATCD_CIM10 = '6.2-avec-colorisation';

    const HOST_WEDA = 'secure.weda.fr';
    const HOST_HEIDI = 'scribe.heidihealth.com';
    const HEIDI_URL = 'https://scribe.heidihealth.com/';

    const KEY_JOB = 'auto_atcd_cim10_job_v1';
    const KEY_LAST_REPORT = 'auto_atcd_cim10_last_report_v1';
    const KEY_WORKER_LOCK = 'auto_atcd_cim10_worker_lock_v1';
    const KEY_WORKER_OPEN_LOCK = 'auto_atcd_cim10_worker_open_lock_v1';
    const KEY_HEIDI_OPEN_LOCK = 'auto_atcd_cim10_heidi_open_lock_v1';
    const KEY_IMPORT_WAKE = 'auto_atcd_cim10_import_wake_v1';
    const KEY_IMPORT_LOGS = 'auto_atcd_cim10_import_logs_v1';
    const KEY_IMPORT_LOG_ENABLED = 'auto_atcd_cim10_import_log_enabled_v1';
    const KEY_COLOR_ONLY_LAST_REPORT = 'auto_atcd_cim10_color_only_last_report_v1';
    const KEY_WEDA_LAUNCHER_PANEL_POSITION = 'auto_atcd_cim10_launcher_panel_position_v1';
    const LOCALSTORAGE_BATCH_REPORT_KEY = 'auto_atcd_cim10_batch_last_report_v1';
    const LOCALSTORAGE_CIM10_BATCH_CLICK_CONTEXT = 'auto_atcd_cim10_batch_click_context_v1';
    const LOCALSTORAGE_BATCH_SOURCE_CLOSE_KEY = 'auto_atcd_cim10_batch_source_close_v1';

    const SESSION_TAB_ID = 'auto_atcd_cim10_tab_id_v1';
    const SESSION_WORKER_JOB_ID = 'auto_atcd_cim10_worker_job_id_v1';
    const SESSION_HEIDI_JOB_ID = 'auto_atcd_cim10_heidi_job_id_v1';
    const SESSION_HEIDI_JOB_HASH_SEEN_AT = 'auto_atcd_cim10_heidi_job_hash_seen_at_v1';

    const WORKER_HASH_PREFIX = 'AUTO_ATCD_CIM10_WORKER=';
    const HEIDI_HASH_PREFIX = 'AUTO_ATCD_CIM10_HEIDI=';
    const WEDA_IMPORT_IFRAME_ID = 'auto-atcd-cim10-weda-import-frame';
    const WEDA_IMPORT_IFRAME_FLAG = 'AUTO_ATCD_CIM10_IFRAME=1';
    const WEDA_IMPORT_IFRAME_ENABLED = false;

    const LOG_PREFIX = '[AUTO-ATCD-CIM10]';
...
END_ATCD`;
    ...
    function publishBatchBridgeReport(job) {
        ...
        const report = {
            ...
            ts: nowMs(),
            bridgeSource: 'antecedents-cim10-weda-heidi-avec-colorisation',
            bridgeVersion: VERSION_AUTO_ATCD_CIM10,
            bridgeTabId: TAB_ID,
    ...
    function recoverStaleWedaExtraction(job, source = '') {
        ...
        if (job.extractingWedaTabId === TAB_ID && window.__AUTO_ATCD_CIM10_WEDA_EXTRACT_RUNNING__) return null;
        ...
        logImportEvent('warning', 'extract_weda_recovery', 'Extraction WEDA bloquée avant ouverture Heidi : relance automatique.', {
            jobId: job.id || '',
            source,
    ...
    function showBadge(message, options = {}) {
        try {
            const id = 'auto-atcd-cim10-badge';
    ...
    function buildCompactImportLogExport(logs = getImportLogs(), options = {}) {
        ...
        const header = {
            kind: 'AUTO_ATCD_CIM10_DEBUG_LOG_COMPACT',
            version: VERSION_AUTO_ATCD_CIM10,
            exportedAt: nowIso(),
            count: list.length,
    ...
    function supersedeActiveImportRun(reason = '') {
        window.__AUTO_ATCD_CIM10_IMPORT_RUN_TOKEN__ = `superseded_${Date.now()}_${Math.floor(Math.random() * 1000000)}`;
        window.__AUTO_ATCD_CIM10_IMPORT_SUPERSEDE_REASON__ = reason;
        window.__AUTO_ATCD_CIM10_IMPORT_RUNNING__ = false;
    }

    function assertImportRunActive(runToken) {
        if (!runToken) return;
        if (window.__AUTO_ATCD_CIM10_IMPORT_RUN_TOKEN__ !== runToken) {
    ...
    function getJobProgressSnapshot(job) {
        ...
        return {
            ...
            runningFlag: !!window.__AUTO_ATCD_CIM10_IMPORT_RUNNING__,
    ...
    function installGlobalErrorLogger() {
        if (window.__AUTO_ATCD_CIM10_GLOBAL_ERROR_LOGGER__) return;
        window.__AUTO_ATCD_CIM10_GLOBAL_ERROR_LOGGER__ = true;
    ...
    function checkWedaImportStall(job) {
        ...
        if (window.__AUTO_ATCD_CIM10_LAST_STALL_LOG_KEY__ !== key) {
            window.__AUTO_ATCD_CIM10_LAST_STALL_LOG_KEY__ = key;
    ...
    function refreshImportLogPanelIfOpen() {
        try {
            const panel = document.getElementById('auto-atcd-cim10-log-panel');
    ...
    function removeImportLogUi() {
        try {
            const button = document.getElementById('auto-atcd-cim10-log-button');
    ...
    function showImportLogPanel() {
        ...
        let panel = document.getElementById('auto-atcd-cim10-log-panel');
        if (panel) {
            refreshImportLogPanelIfOpen();
            return panel;
        }
        ...
        panel.id = 'auto-atcd-cim10-log-panel';
    ...
    function injectImportLogButtonIfUseful() {
        try {
            const existing = document.getElementById('auto-atcd-cim10-log-button');
            if (!canShowImportLogUiHere()) {
                removeImportLogUi();
                return;
            }
            ...
            btn.id = 'auto-atcd-cim10-log-button';
            btn.type = 'button';
    ...
    function rememberWorkerJobIdFromHash() {
        ...
        if (id) {
            window.__AUTO_ATCD_CIM10_FRAME_WORKER_JOB_ID__ = id;
            if (isTopLevelWindow()) {
    ...
    function getWorkerJobIdForThisTab() {
        ...
        if (window.__AUTO_ATCD_CIM10_FRAME_WORKER_JOB_ID__) return window.__AUTO_ATCD_CIM10_FRAME_WORKER_JOB_ID__;
    ...
    function clearWorkerJobIdForThisTab() {
        try { window.__AUTO_ATCD_CIM10_FRAME_WORKER_JOB_ID__ = ''; } catch (_) {}
        if (isTopLevelWindow()) {
    ...
    function shouldCloseScriptedHeidiWorker(job) {
        ...
        if (window.__AUTO_ATCD_CIM10_HEIDI_FINALIZE_RUNNING__) return false;
    ...
    async function openHeidiWorkerForJob(job) {
        ...
        const latest = getJob() || job;
        if (!latest || latest.id !== job.id || latest.status !== 'PENDING_HEIDI') return false;
        if (hasActiveHeidiWorkerOrOpening(latest)) return false;

        const reservedOpen = await acquireHeidiWorkerOpenLock(latest);
        if (!reservedOpen) return false;

        const opened = openTabSafe(buildHeidiWorkerUrl(HEIDI_URL, latest.id), !HEIDI_WORKERS_OPEN_IN_BACKGROUND, {
            insert: !HEIDI_WORKERS_OPEN_IN_BACKGROUND,
            setParent: true
        });

        if (!opened) {
            releaseHeidiWorkerOpenLock(latest);
            return false;
        }

        markHeidiWorkerOpened(latest);
        return true;
    }
    ...
    function closeCurrentWedaWorkerTab(job) {
        ...
        window.__AUTO_ATCD_CIM10_WEDA_CLOSING__ = true;
        clearWorkerJobIdForThisTab();
    ...
    function closeCurrentHeidiTabSoon(message = 'Résultat transmis à WEDA.\nFermeture de l’onglet Heidi…', options = {}) {
        if (!isHeidi()) return;
        if (window.__AUTO_ATCD_CIM10_HEIDI_CLOSING__) return;

        window.__AUTO_ATCD_CIM10_HEIDI_CLOSING__ = true;
    ...
    function installWedaLauncherPanelDrag(panel, handle) {
        if (!panel || !handle || panel.__AUTO_ATCD_CIM10_DRAG_INSTALLED__) return;
        panel.__AUTO_ATCD_CIM10_DRAG_INSTALLED__ = true;
    ...
    function injectWedaButtonIfUseful() {
        ...
        const panelId = 'auto-atcd-cim10-launcher-panel-avec-colorisation';
        const mainButtonId = 'auto-atcd-cim10-launcher-avec-colorisation';
        const colorOnlyButtonId = 'auto-atcd-cim10-color-only-launcher';
        ...
        handle.textContent = `ATCD CIM-10 ${VERSION_AUTO_ATCD_CIM10.replace(/-avec-colorisation$/, '')}`;
        ...
        mainBtn.textContent = 'Heidi CIM10 + couleur';
        mainBtn.title = 'Exporter les antécédents WEDA non codés vers Heidi Ask AI, importer les CIM-10 puis coloriser localement. Raccourci : touche Début / Home';
        ...
        colorOnlyBtn.title = 'Coloriser les antécédents CIM-10 déjà présents dans WEDA, sans lancer Heidi ni coder de nouveaux antécédents.';
    ...
    function injectHeidiImportButtonIfUseful() {
        ...
        const existing = document.getElementById('auto-atcd-cim10-heidi-import');
        ...
        const btn = document.createElement('button');
        btn.id = 'auto-atcd-cim10-heidi-import';
        btn.type = 'button';
    ...
    function isWedaAutoUiElement(el) {
        try {
            return !!(
                el &&
                el.closest &&
                el.closest('#auto-atcd-cim10-badge, #auto-atcd-cim10-log-button, #auto-atcd-cim10-log-panel, #auto-atcd-cim10-launcher-panel-avec-colorisation')
            );
        } catch (_) {
            return false;
        }
...
${HEIDI_ASK_AI_PROMPT}`
    ...
    async function handleWedaJob() {
        ...
        if (window.__AUTO_ATCD_CIM10_WEDA_EXTRACT_RUNNING__) return;

        window.__AUTO_ATCD_CIM10_WEDA_EXTRACT_RUNNING__ = true;
        ...
        try {
            ...
            if (!antecedentRoot) {
                ...
                logImportEvent('error', 'extract_weda_dom_missing', 'Page Antécédents ouverte, mais structure WEDA introuvable avant ouverture Heidi.', {
                    jobId: job.id,
                    url: location.href,
                    isAntecedentUrl: isAntecedentUrlWeda(),
                    updatePanelAntecedent: !!getWedaAntecedentRoot()
                });
                showBadge('Page Antécédents détectée, mais contenu WEDA non chargé.\nHeidi non lancé pour éviter une extraction vide.', { error: true, duration: 12000 });
                return;
            }
            ...
            if (deletionReport.deletedCount > 0) {
                showBadge(`${deletionReport.deletedCount} antécédent parasite supprimé avant Heidi.`, { duration: 6000 });
                await sleep(900);
            }
            ...
            showBadge(`${items.length} bloc(s) d’antécédents non codés récupéré(s).\nOuverture de Heidi en arrière-plan…`, { duration: 8000 });
            const opened = await openHeidiWorkerForJob(job);
            if (!opened) {
                showBadge('Heidi est déjà ouvert pour ce job.\nAucun nouvel onglet lancé.', { duration: 6000 });
            }
        } finally {
            window.__AUTO_ATCD_CIM10_WEDA_EXTRACT_RUNNING__ = false;
        }
    }
    ...
    function canFinalizeVisibleHeidiJob(job) {
        ...
        if (status !== 'RUNNING_HEIDI') return false;
    ...
    function maybeMarkUnparsedHeidiResult(job, visible, source = 'heidi_watcher') {
        if (!job || job.status !== 'RUNNING_HEIDI') return false;
        ...
        const latest = getJob() || job;
        if (!latest || latest.id !== job.id || latest.status !== 'RUNNING_HEIDI') return false;
    ...
    async function finalizeHeidiResultAndOpenWeda(resultText, source = 'auto', options = {}) {
        if (window.__AUTO_ATCD_CIM10_HEIDI_FINALIZE_RUNNING__) {
            showBadge('Transmission Heidi déjà en cours dans cet onglet.', { duration: 4000 });
            return getJob();
        }

        window.__AUTO_ATCD_CIM10_HEIDI_FINALIZE_RUNNING__ = true;
        ...
        try {
            ...
            if (heidiRunnerIsActiveForOtherTab(current)) {
                if (!forceRunner) {
                    showBadge('Un autre onglet Heidi pilote ce job.\nCet onglet reste inactif.', { duration: 6000 });
                    if (isScriptedHeidiWorkerTab()) {
                        closeCurrentHeidiTabSoon('Un autre onglet Heidi pilote ce job.\nFermeture de ce doublon…', {
                            unloadIfBlocked: true,
                            delayMs: 4000
                        });
                    }
                ...
                logImportEvent('warning', 'heidi_runner_takeover', 'Reprise du verrou Heidi pour finaliser un résultat visible et stable.', {
                    jobId: current.id || '',
                    source,
                    currentTabId: TAB_ID,
            ...
            if (alreadyImporting && hasActiveWedaWorkerOrOpening(current)) {
                showBadge('Résultat Heidi déjà transmis à WEDA.\nImport déjà en cours.', { duration: 7000 });
                closeCurrentHeidiTabSoon('Import WEDA déjà lancé.\nFermeture de cet onglet Heidi…');
                return current;
            }

            const allowedStatuses = ['PENDING_HEIDI', 'RUNNING_HEIDI', 'IMPORT_WEDA', 'ERROR'];
            if (current.status && !allowedStatuses.includes(current.status)) {
                showBadge('Résultat Heidi détecté, mais aucun job actif compatible.', { error: true, duration: 8000 });
                return null;
            }
            ...
            if (!parsedItems.length) {
                logImportEvent('warning', 'parse_heidi_result', 'Résultat Heidi récupéré, mais aucun antécédent CIM10 exploitable à importer.', {
                    jobId: job.id,
                    skipped: skippedLines
                });
                showBadge('Résultat Heidi récupéré, mais aucun antécédent avec CIM10 exploitable à importer.', { error: true, duration: 12000 });
                return job;
            }

            const importUrl = job.wedaImportUrl || job.patientUrl || 'https://secure.weda.fr/';
            const workerUrl = buildWedaWorkerUrl(importUrl, job.id);
            logImportEvent('info', 'parse_heidi_result', `${parsedItems.length} antécédent(s) CIM10 prêt(s) pour import WEDA.`, {
                jobId: job.id,
                source,
            ...
            if (!reservedOpen) {
                const latest = getJob() || job;
                showBadge('Résultat déjà pris en charge par un autre onglet Heidi.\nImport WEDA en cours.', { duration: 6000 });
            ...
            if (!opened) {
                releaseWedaWorkerOpenLock(job);
                showBadge('Impossible d’ouvrir automatiquement WEDA worker.\nLance AUTO_ATCD_CIM10_FORCE_IMPORT_FROM_HEIDI() ou ouvre WEDA manuellement.', { error: true, duration: 15000 });
                return getJob() || job;
            }
            ...
            window.__AUTO_ATCD_CIM10_HEIDI_FINALIZE_RUNNING__ = false;
        }
    }
    ...
    async function handleHeidiJob() {
        ...
        if (!job || job.status !== 'PENDING_HEIDI') return;
        ...
        try {
            ...
            if (resultText && looksLikeHeidiCim10Result(resultText)) {
                ...
                if (looksLikeHeidiCim10Result(refreshed)) {
                    ...
                    if (!current || current.id !== job.id || current.status !== 'RUNNING_HEIDI') {
                        return;
                    }
                    current.status = 'RUNNING_HEIDI';
                    current.updatedAt = nowIso();
    ...
    async function startWedaColorOnly(source = 'button') {
        ...
        try {
            ...
            window.__AUTO_ATCD_CIM10_COLOR_ONLY_RUNNING__ = false;
        }
    }
    ...
    async function handleWedaImportJob() {
        ...
        if (window.__AUTO_ATCD_CIM10_IMPORT_RUNNING__) return;
        ...
        window.__AUTO_ATCD_CIM10_IMPORT_RUNNING__ = true;
        const runToken = createImportRunToken();
        window.__AUTO_ATCD_CIM10_IMPORT_RUN_TOKEN__ = runToken;
    ...
    function installKeyboardShortcut() {
        window.addEventListener('keydown', (event) => {
            ...
            if (isHomeKey) {
                event.preventDefault();
                event.stopPropagation();
                showBadge('Touche Début détectée.\nLancement export ATCD WEDA vers Heidi Ask AI…', { duration: 5000 });
    ...
    function exposeConsoleFunctions() {
        const api = {
            AUTO_ATCD_CIM10_START: () => startAtcdCim10ExportFromWeda('console'),

            AUTO_ATCD_CIM10_COLOR_ONLY: () => startWedaColorOnly('console'),

            AUTO_ATCD_CIM10_COLOR_ONLY_LAST: () => {
            ...
            AUTO_ATCD_CIM10_LOG_ON: () => setImportLogEnabled(true),

            AUTO_ATCD_CIM10_LOG_OFF: () => setImportLogEnabled(false),

            AUTO_ATCD_CIM10_LOG_SHOW: () => showImportLogPanel(),

            AUTO_ATCD_CIM10_LOGS: () => {
                const logs = getImportLogs();
            ...
            AUTO_ATCD_CIM10_LOGS_RAW: () => {
                const logs = getImportLogs();
                console.log(LOG_PREFIX, 'IMPORT_LOGS_RAW', logs);
                return logs;
            },

            AUTO_ATCD_CIM10_LOG_CLEAR: () => clearImportLogs(),

            AUTO_ATCD_CIM10_ERRORS: () => {
            ...
            AUTO_ATCD_CIM10_DIAG_ALLERGIES: () => {
                const report = {
                    at: nowIso(),
                    url: location.href,
            ...
            AUTO_ATCD_CIM10_CLICK_NO_ALLERGY_NOW: async () => {
            ...
            AUTO_ATCD_CIM10_READ_HEIDI_RESULT: () => {
                const text = getHeidiAskAiText();
            ...
            AUTO_ATCD_CIM10_TEST_PARSE_HEIDI_RESULT: (text) => {
            ...
            AUTO_ATCD_CIM10_TEST_COLLATERAL: (member) => {
                const value = mapFamilyMemberToCollateralValue(member);
                const report = {
                    member,
                    value,
            ...
            AUTO_ATCD_CIM10_TEST_COLLATERAL_LABELS: (member, labels) => {
            ...
            AUTO_ATCD_CIM10_DIAG_COLLATERAL: (member = 'Tante') => {
            ...
            AUTO_ATCD_CIM10_TEST_COMMENT: (description, remarks, date) => {
                const details = {
            ...
            AUTO_ATCD_CIM10_TEST_PARENT_CODES: (code) => {
                const report = {
                    code,
            ...
            AUTO_ATCD_CIM10_TEST_SIMILARITY: (reference, candidate) => {
                const report = {
                    reference,
                    candidate,
            ...
            AUTO_ATCD_CIM10_TEST_DUPLICATE_FUZZY: (reference, candidate, codeA = 'N10', codeB = codeA, dateA = '', dateB = '') => {
                const item = {
                    section: 'medical',
            ...
            AUTO_ATCD_CIM10_TEST_DUPLICATE_HIGH_COMMENT: (reference, candidate, codeA = 'N10', codeB = 'I10', dateA = '', dateB = '', familyA = '', familyB = '') => {
            ...
            AUTO_ATCD_CIM10_TEST_HEIDI: async (text) => {
                ...
                const job = {
                    ...
                    createdAt: nowIso(),
                    updatedAt: nowIso(),
                    source: 'console_test_heidi',
                    sourceWedaTabId: '',
                    wedaWorkerTabId: '',
                    status: 'PENDING_HEIDI',
                    itemCount: 0,
                    extractedText: sample,
                    heidiPayload: buildHeidiPayload(sample),
                    errors: []
                };
    ...
    function installWedaImportWakeListener() {
        if (!isWeda()) return;
        if (window.__AUTO_ATCD_CIM10_IMPORT_WAKE_LISTENER__) return;
        window.__AUTO_ATCD_CIM10_IMPORT_WAKE_LISTENER__ = true;
        ...
        setInterval(() => {
            try {
                ...
                if (
                    ...
                    if (recovered) {
                        showBadge('Extraction WEDA relancée automatiquement avant Heidi…', { duration: 5000 });
                ...
                if (
                    ...
                    runWatchedAsync('weda_pending_heidi_open_async', () => openHeidiWorkerForJob(job), { job });
                }
